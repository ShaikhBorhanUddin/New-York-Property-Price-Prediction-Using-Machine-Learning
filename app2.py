...
block_lot_mapping_path = 'Models/block_lot_mapping.pkl'
...
def get_valid_blocks():
    return sorted({
        k[3] for k in block_lot_mapping.keys()
        if k[0] == st.session_state.selected_borough_id
        and k[1] == st.session_state.selected_neighborhood_name
        and k[2] == st.session_state.selected_zip_code
    })

def get_valid_lots(block):
    return sorted(
        block_lot_mapping.get(
            (
                st.session_state.selected_borough_id,
                st.session_state.selected_neighborhood_name,
                st.session_state.selected_zip_code,
...
def on_zip_code_change():
    ...
    blocks = get_valid_blocks()
    st.session_state.selected_block = blocks[0] if blocks else 0

    lots = get_valid_lots(st.session_state.selected_block)
    st.session_state.selected_lot = lots[0] if lots else 0

    update_lat_lon()

def on_block_change():
    st.session_state.selected_block = st.session_state.block_select
    lots = get_valid_lots(st.session_state.selected_block)
    st.session_state.selected_lot = lots[0] if lots else 0
...
if 'selected_block' not in st.session_state:
    st.session_state.selected_block = 374

if 'selected_lot' not in st.session_state:
    st.session_state.selected_lot = 46
...
with col1:
    ...
    st.selectbox(
        "BLOCK",
        options=blocks,
        index=get_select_box_index(blocks, st.session_state.selected_block),
        key="block_select",
    ...
    st.selectbox(
        "LOT",
        options=lots,
        index=get_select_box_index(lots, st.session_state.selected_lot),
        key="lot_select"
    )
